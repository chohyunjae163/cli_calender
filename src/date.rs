...
//Jan. 1st of the Gregorian calendar is Thursday in 1582.
...
pub fn run() {
    //print hello calendar title ascii code
    print_title();
...
fn calc_day(date: &Date) -> u32 {
    ...
    let mut extra_days_for_leap_years = leap_year_count;
    //if the year is leap year, minus one.
    if is_leap_year(date.year) {
        extra_days_for_leap_years -= 1;
    }
    const YEAR: u32 = 365;
    days += (date.year - GREGORIAN_CALENDAR) * YEAR + date.day + extra_days_for_leap_years;
    days % 7
}
